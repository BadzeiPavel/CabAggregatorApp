...
public class AuthController {
    ...
    @PostMapping("/login")
    public Mono<ResponseEntity<String>> login(@RequestBody AuthUserDTO authUserDTO) {
        return authService.login(authUserDTO);
    }

    @PostMapping("/register")
    public Mono<ResponseEntity<User>> register(@RequestBody AuthUserDTO authUserDTO) {
        return Mono.just(ResponseEntity.status(HttpStatus.CREATED)
                .body(authService.register(authUserDTO)));
    }

    @PostMapping("/logout")
    public Mono<ResponseEntity<String>> logout(@RequestBody LogoutDTO logoutDTO) {
